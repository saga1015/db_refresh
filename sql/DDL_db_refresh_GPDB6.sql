...
CREATE VIEW v_list_dump_tables AS
    SELECT refresh_list_dump_table.dump_timestampkey, refresh_list_dump_table.dump_line AS table_name 
...
CREATE VIEW v_list_table_distrib_key AS
	...
	FROM (SELECT d.localoid AS reloid, n.nspname AS schema_name, c.relname AS table_name, string_agg(quote_ident((a.attname)::text), ', '::text ORDER BY d.colorder) AS distrib_column_list 
		FROM (((((SELECT gp_distribution_policy.localoid, unnest(gp_distribution_policy.distkey) AS colnum, generate_series(1, 1+array_upper(gp_distribution_policy.distkey, 1)) AS colorder 
				FROM gp_distribution_policy 
				WHERE (gp_distribution_policy.distkey IS NOT NULL)) d 
				JOIN pg_attribute a 
			...
			UNION ALL 
				...
				LEFT JOIN pg_partitions part 
					ON (((n.nspname = part.partitionschemaname) AND (c.relname = part.partitiontablename)))) 
				WHERE (((part.partitiontablename IS NULL) AND (d.distkey = '')) AND (c.relstorage <> 'x'::"char"))
		) t;
